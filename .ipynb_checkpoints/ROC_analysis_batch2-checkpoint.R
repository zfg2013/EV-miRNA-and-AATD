...
WD_roc <- file.path(WD_DE, "roc_batch2")

dir.create(WD_roc)

afile <- paste0("DE_","Table_batch2", ".csv")
...
data_count0 <- data_count0_batch2 <- get(load(file.path(WD_data_batch2, "count_no.rdata")))
sample_existance <- tibble(ID=colnames(data_count0), exist=1)
pheno_edata <- sample_existance %>% inner_join(pheno_ddata) %>% mutate(PASD_binary = ifelse(PASD <= 1, 0, 1))
...
## visualization
setwd(WD_roc)
write.csv(DEresult4, "DE_roc_batch2.csv", row.names = FALSE)
